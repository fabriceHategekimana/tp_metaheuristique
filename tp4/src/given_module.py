def cities_coordinates(filename):
    ...
    with open(filename, "r") as f:
        for li in f:
            line = list(filter(None, li.rstrip().split(' ')))
...
def compute_distances(setting):
    ...
    of a setting (coordinates of cities)
    ...
    NOTE:
    1. You can implement by computing the distances everytime,
    but since the distance between 2 cities will be needed frequently,
    this step saves computations.
    2. In case you use this dictionary implementation, remember that
    distance(c1, c2) = distance(c2, c1) --> even less calculations
    (but more memory !)
    '''
...
def prob_to_city(i, j, J, dist, T, alpha, beta):
    ...
    beta : parameter that controls the relative importance of
    the heuristic information η_ij
